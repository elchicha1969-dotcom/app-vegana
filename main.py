...
def main(page: ft.Page):
    ...
    # Función corregida (sin .update() prematuro)
    def ajustar_etiquetas():
        if estado["seccion_actual"] == 2: # Restaurantes / Sitios
            txt_desc.label = "Lugar"
        else: # Recetas o Productos
            txt_desc.label = "Descripción"
        # Quitamos txt_desc.update() para evitar el error de Assertion
    ...
    # --- LISTA VISUAL (NUEVO DISEÑO HORIZONTAL) ---
    def obtener_lista_visual(clave_db, color_tag, icono_defecto):
        ...
        if not datos:
            ...
            for item in datos:
                ...
                if item.get("video"):
                    tarjeta.content.on_click = lambda e, url=item["video"]: page.launch_url(url)
