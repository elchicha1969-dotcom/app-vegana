...
def main(page: ft.Page):
    ...
    # --- CAPAS DE FONDO (USANDO STACK PARA EVITAR ERROR) ---
    ...
    # --- LISTA VISUAL (DISEÑO STACK SIN ERROR) ---
    def obtener_lista(clave_db, color_tag, icono):
        ...
        if not datos:
            ...
            for item in datos:
                ...
                bg_overlay = "#80000000" if tiene_imagen else "#FFFFFF" # Fondo oscuro transp. si hay foto
                ...
                # --- ARREGLO AQUÍ: Usamos Stack en lugar de image_src ---
                stack_card = []
                ...
                # Capa 2: Contenido + Fondo semitransparente
                stack_card.append(ft.Container(bgcolor=bg_overlay, padding=15, content=info, expand=True))

                tarjeta = ft.Card(
                    elevation=5, color="white", margin=ft.margin.symmetric(horizontal=10), clip_behavior=ft.ClipBehavior.ANTI_ALIAS,
                    # Altura fija para que se vea bien la foto de fondo
                    content=ft.Container(height=280, content=ft.Stack(controls=stack_card))
                )
