...
def main(page: ft.Page):
    # --- CONFIGURACIÓN PARA MÓVIL REAL ---
    page.title = "Vida Vegana"
    page.theme_mode = "light"
    page.padding = 0  # Sin márgenes, pantalla completa
    page.theme = ft.Theme(color_scheme_seed="#388E3C")
    # Nota: Hemos quitado window_width y window_height porque el móvil decide el tamaño
    ...
    def ejecutar_borrado_final(clave_db, id_a_borrar):
        lista_actual = page.client_storage.get(f"mis_{clave_db}") or []
        ...
        if len(lista_nueva) < len(lista_actual):
            ...
            page.show_snack_bar(ft.SnackBar(ft.Text("No se encontró el item."), bgcolor="orange"))
    ...
    dialogo_login = ft.AlertDialog(
        ...
        content=ft.Column([ft.Text("Introduce PIN:", size=14), campo_pin], height=100),
        actions=[
    ...
    def obtener_lista_visual(clave_db, color_tag, icono_defecto):
        ...
        if not datos:
            columna.controls.append(ft.Container(alignment=ft.alignment.center, padding=20, content=ft.Container(padding=30, width=320, bgcolor="#99FFFFFF", border_radius=15, content=ft.Column([ft.Icon("info", size=40, color="grey"), ft.Text("Lista vacía", color="grey", weight="bold"), ft.Text("Usa (+) para añadir.", color="grey", size=12)], horizontal_alignment="center"))))
        else:
            for item in datos:
                this_id = item.get("id")
                imagen_item = ft.Container()
                if item.get("imagen"):
                    imagen_item = ft.Image(src=item["imagen"], width=400, height=150, fit=ft.ImageFit.COVER, border_radius=ft.border_radius.vertical(top=10), error_content=ft.Icon("broken_image", color="grey"))

                detalles = ft.Container()
                if item.get("contenido"):
                    detalles = ft.ExpansionTile(title=ft.Text("Ver Detalles", color="#388E3C", size=14), controls=[ft.Container(padding=10, content=ft.Text(item["contenido"], size=13, color="#424242"))])
                ...
                delete_btn = ft.IconButton(icon="delete_forever", icon_color="red", icon_size=30, tooltip="Borrar item", on_click=lambda e, id_ref=this_id: solicitar_borrado(clave_db, id_ref))

                tarjeta = ft.Card(width=320, elevation=5, color="#F2FFFFFF", clip_behavior=ft.ClipBehavior.ANTI_ALIAS, content=ft.Container(content=ft.Column([imagen_item, ft.Container(padding=15, content=ft.Column([ft.Row([ft.Icon(icono_defecto, size=24, color="#388E3C"), ft.Text(item["titulo"], weight="bold", size=18, color="black", expand=True), ft.Container(content=ft.Text(item["tag"], size=10, color="white", weight="bold"), bgcolor=color_tag, padding=5, border_radius=5)], alignment="spaceBetween"), ft.Text(item["desc"], size=13, color="#616161"), ft.Divider(height=10, color="transparent"), detalles, ft.Row([link_btn, ft.Container(expand=True), delete_btn], alignment="spaceBetween")]))], spacing=0)))
    ...
    vista_formulario = ft.Container(bgcolor="white", padding=20, width=320, border_radius=15, alignment=ft.alignment.top_center, content=ft.Column([ft.Text("Añadir Nuevo", size=24, weight="bold", color="#388E3C"), ft.Divider(), txt_nombre, txt_desc, txt_tag, ft.Divider(), ft.Row([txt_img, ft.IconButton("photo_library", icon_color="#388E3C", on_click=lambda _: picker.pick_files())]), txt_vid, txt_cont, ft.Container(height=20), ft.Row([ft.ElevatedButton("Cancelar", on_click=cancelar, bgcolor="grey", color="white"), ft.ElevatedButton("GUARDAR", on_click=guardar, bgcolor="#388E3C", color="white")], alignment="center"), ft.Container(height=30)], scroll="auto"))
    ...
    def mostrar_seccion(indice):
        ...
        if indice == 0:
            ...
            contenido_inicio = ft.Container(bgcolor="#99FFFFFF", padding=30, width=300, border_radius=20, content=ft.Column([ft.Icon("eco", size=80, color="#388E3C"), ft.Text("Bienvenido", size=30, weight="bold", color="#388E3C"), ft.Text("Tu espacio vegano.", size=16, color="#111111", text_align="center"), ft.Container(height=20), ft.Text("👇 Elige una opción abajo", size=14, weight="bold", color="#388E3C")], horizontal_alignment="center", spacing=5))
    ...
    # --- MONTAJE NATIVO (SIN MARCO NEGRO) ---
    ...
    app_bar = ft.Container(
        ...
        content=ft.Row([ft.Row([ft.Icon("eco", color="white"), titulo]), btn_lock], alignment="spaceBetween")
    )

    # AQUÍ ESTÁ EL CAMBIO: Ya no usamos "celular_simulado"
    # Usamos una Columna principal que llena la página
    layout_principal = ft.Column(
        spacing=0,
        expand=True,
        controls=[
            app_bar,
            ft.Container(content=stack_principal, expand=True), # El contenido se expande
            nav
        ]
    )

    page.add(layout_principal)
    # El botón flotante ahora usa la propiedad nativa de la página
