        ...
        if not datos:
            ...
            for item in datos:
                ...
                info = ft.Column([
                    ...
                    ft.Text(item["desc"], size=13, color=color_texto), # Color corregido para que se vea
                    ft.Divider(height=10, color="white24" if tiene_imagen else "black12"),
                    extras, actions
                ])

                # --- ARREGLO IMAGEN ADAPTADA: CONTAIN ---
                stack_card = []
                # 1. Fondo Oscuro (para rellenar huecos)
                stack_card.append(ft.Container(bgcolor="#222222", expand=True))
                
                # 2. Imagen en modo CONTAIN (Se ve entera)
                if tiene_imagen:
                    stack_card.append(ft.Image(src=src, fit=ft.ImageFit.CONTAIN, opacity=0.8, expand=True))
